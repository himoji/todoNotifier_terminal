...
pub fn time_parser(s: String) -> i64 {
    ...
    if s.contains("now") {
        ...
        now + chrono::Duration::hours(
            string_work::extract_number(h).expect("Couldn't extract number from query") as i64,
        )
        .num_seconds()
            + chrono::Duration::minutes(
                string_work::extract_number(m).expect("Couldn't extract number from query") as i64,
            )
            .num_seconds()
    } else {
        string_work::extract_number(s.as_str()).expect("Couldn't extract number from query") as i64
    }
}
